...
public interface BookRepo extends JpaRepository<Book, Integer> {
    ...
    // TODO: implement logic to find the Book of the Month. Right now it gets the
    // most expensive book.
    @Query(value = "select book.* from book,bookdetails,format where format.formatid=bookdetails.formatid and book.bookid=bookdetails.bookid \n"
            +
    ...
    // " where book.bookid=joinedbookauthor.bookid and
    // author.authorid=joinedbookauthor.authorid\n" +
    // " and( author.firstname LIKE Upper('%?1%') OR author.lastname LIKE
    // Upper('%?1%')) limit 0,5;",nativeQuery=true)
    // public List<Book>
    // findByAuthorFirstNameContainingOrLastNameContainingIgnoreCase( String term);

    // List<Book> basedOnPaperBack;
    /*
     * select bookdetailsid from cartitem group by bookdetailsid order by
     * SUM(quantity) desc limit 0,5;
     * Gia ta bestsellers
     */

    // axreiasto, afou exw ta booksPerCategory, arkei na tsimphsw to onoma, kai na
    // to balw sto path
    // @Query(value="Select * from book,joinedbookcategory,category\n" +
    // "where book.bookid=joinedbookcategory.bookid and
    // category.categoryid=joinedbookcategory.categoryid \n" +
